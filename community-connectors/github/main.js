...
function applyGetSchema(connector) {
	...
	connector.getSchema = function getSchema() { return schema; };
...
function makeGetConfig(config) {
	var normalizedConfig = config instanceof Array ?
		{ configParams: config, dateRangeRequired: false } :
		config;

	return function getConfig() { return normalizedConfig; };
};
...
function encodeQuery(queryParams) {
	...
	var query = Object.keys(queryParams).map(function (key) {
...
function combineConnectors(options) {
	...
	var connectorOptionDef = {
		...
		options: mapObjectSorted(connectorDefs, function (connector, connectorKey) {
			return {
				value: connectorKey,
	...
	var getFullConfig = function getFullConfig(request) {
		var userConfig = getConfig(request);
		return {
	...
	var validateFullConfig = function validateFullConfig(configParams) {
		...
		if (!connectors[configParams[connectorOptionKey]]) {
			throw new UserError("Invalid Connector selected: " + configParams[connectorOptionKey]);
		}
	};
...
var githubStarsConnector = applyGetSchema({
  ...
  */
	paginatedUrl: function paginatedUrl(pageNumber, organization, repository) {
  ...
  */
	fetchURL: function fetchURL(request, pageNumber) {
		var organization = request.configParams.organization;
  ...
  */
	numberOfPages: function numberOfPages(initialResponse) {
