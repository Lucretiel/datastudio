...
connector.customConfig = [{
  ...
  helpText: 'Enter the name of the organization for which you would like to retrieve information.',
  placeholder: 'google'
}, {
  name: 'Repository',
  displayName: 'Repository',
  helpText: 'Enter the name of the repository for which you would like to retrieve information.',
...
connector.validateConfig = function (configParams) {
  ...
  configParams.Organization = configParams.Organization || connector.throwError('You must provide an Organization.', true);

  configParams.Repository = configParams.Repository || connector.throwError('You must provide a Repository.', true);
};
...
connector.paginatedUrl = function (pageNumber, organization, repository) {
  var urlParts = ['https://api.github.com/repos/', organization, '/', repository, '/stargazers',
  // URL Parameters
...
function getOAuthService() {
  ...
  return OAuth2.createService('github').setAuthorizationBaseUrl('https://github.com/login/oauth/authorize').setTokenUrl('https://github.com/login/oauth/access_token').setClientId(clientId).setClientSecret(clientSecret).setPropertyStore(PropertiesService.getUserProperties()).setCallbackFunction('authCallback');
}
